...
function init() {
	...
	scene.background = new THREE.Color(0x5d6166);
...
// Materials: points, line and solid
...
function Animation3() {
	...
	if (positiony < position_y + 30 && t == 0) 
	{ 
		mesh.position.y += 0.1;
		mesh.rotation.y += 0.025;
	}
	if (positiony > position_y + 30) {
		t += 1;
	}

	if (t > 1 && positiony > position_y) 
	{ 
		mesh.position.y -= 0.1;
		mesh.rotation.y += 0.025;
	}
	...
	render();
	id_animation4 = requestAnimationFrame(Animation3);
}
...
function Animation4() {
	...
	var positiony = mesh.position.y;
	if (positiony < position_y + 30 && k == 0)
	{
		mesh.position.y += 0.1;
	}
	...
	if (k > 1 && positiony > position_y)
	{
		mesh.position.y -= 0.1;
	}
	if (k > 1 && positiony < position_y && positionx > position_x)
	{
		mesh.position.x -= 0.1;
	}
	if (positiony < position_y && positionx < position_x) {
		k = 0;
	} 
	mesh.rotation.y += 0.01;
	render();
